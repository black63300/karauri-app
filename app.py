...
st.info(f"📉 現在【{st.session_state.refresh_min}分間隔】で自動更新中！空売り比率が高い銘柄をハックしてね✨ [cite: 2025-11-29, 2025-12-20]")
    ...
    for i in range(0, len(res), 5):
        ...
        for j, (idx, row) in enumerate(chunk.iterrows()):
            with cols[j]:
                ...
                icon = "🔺" if row['前日比'] > 0 else "🔻" if row['前日比'] < 0 else "💨"
                st.markdown(f"""
                    <div class="tile-item" style="border: 1.5px solid {color};">
                        ...
                        <div style="color:{chg_c};font-size:0.75rem;font-weight:bold;">{icon} {abs(row['前日比'])}%</div>
                    </div>
                """, unsafe_allow_html=True)
...
if st.session_state.selected_ticker:
    ...
    try:
        ...
        hist = yf.Ticker(f"{ct}{sfx}").history(period="1mo")
        if not hist.empty:
            fig = go.Figure(data=[go.Candlestick(
                x=hist.index, open=hist['Open'], high=hist['High'], low=hist['Low'], close=hist['Close'],
                increasing_line_color='#ff00ff', decreasing_line_color='#00ffff'
            )])
            fig.update_layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', margin=dict(l=10, r=10, t=10, b=10), height=380, xaxis=dict(showgrid=False, tickfont=dict(color="#888"), rangeslider=dict(visible=False)), yaxis=dict(showgrid=True, gridcolor="#222", tickfont=dict(color="#888")))
...
with st.container():
    ...
    if st.session_state.selected_ticker:
        try:
            tg = st.session_state.selected_ticker
            ctg = str(tg)[:4] if st.session_state.market_type == "JPN" else tg
            sfx = ".T" if st.session_state.market_type == "JPN" else ""
