...
# --- 2. セッション管理 & 更新 (1-60分) ---
for k, v in {'market': 'JPN', 'segment': 'ALL', 'usa_seg': 'TECH', 'target_ticker': '9984.T', 'pinned_ticker': '9984.T', 'refresh_min': 5}.items():
...
with st.sidebar:
    ...
    st.write(f"今日で **{days_met}日目** だぬ！ [cite: 2025-11-30]")
    st.session_state.refresh_min = st.slider("🕒 自動更新（分）", 1, 60, st.session_state.refresh_min)

# --- 3. メインモニター (市場選択を維持！ ✨) ---
st.title(f"🕶️ {st.session_state.market} 空売り監視モニター")
st.info(f"📊 {START_DATE}から{days_met}日目！ BLACK、ハックしよ💖")
...
def get_shorts_data(m_type, j_seg, u_seg):
    try:
        ...
        # APIが死んでる時は警告を出すぬ！ 🚨
        st.error(f"APIハック失敗だぬ... トークンを確認してね💦")
        return pd.DataFrame([{"コード": f"SAMPLE_{i}", "比率": 0.0} for i in range(15)])
...
# --- 5. Charting (左右分離 & ローソク足復活 ✨) ---
st.divider()
c1, c2 = st.columns(2)

def draw_god_chart(t, chart_id, title):
    try:
        ...
        if not h.empty:
            # 💡 MultiIndexを潰して確実に描画するぬ！ [cite: 2025-11-29]
...
with c1:
    ...
    if st.button("📌 右側に固定！", type="primary"): st.session_state.pinned_ticker = st.session_state.target_ticker; st.rerun()
    draw_god_chart(st.session_state.target_ticker, "left", "ANALYSIS")

with c2:
    st.write(f"📍 PINNED: **{st.session_state.pinned_ticker}**")
    draw_god_chart(st.session_state.pinned_ticker, "right", "PINNED")
